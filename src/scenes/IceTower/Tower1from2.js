...
export default class IceTower1from2 extends Phaser.Scene
{
    // player;
    preload()
    {
    ...
    create()
    {
        ...
        .on('pointerdown', () => { 
            let paused = true
            this.scene.launch("pauseScreen",{scene:"icetower1from2"})
            this.scene.sleep("icetower1from2")
    ...
    update() {
        ...
        if (character.x > 785) {
            this.scene.start("icestartingPointfrom1")
        }
        if (character.y < 5) {
            this.scene.start("icetower2_4")
        }
    }
}