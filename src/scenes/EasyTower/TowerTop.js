...
export default class EasyTowerTop extends Phaser.Scene
{
    // player;
    preload()
    {
    ...
    create()
    {
        ...
        .on('pointerdown', () => { 
            let paused = true
            this.scene.launch("pauseScreen",{scene:"easytowertop"})
            this.scene.sleep("easytowertop")
    ...
    update() {
        ...
        if (character.x < 5) {
            this.scene.start("easytower5fromtop")
        }
        if (character.y < 20) {
            this.scene.start("easyendingPoint")
        }
    }
}