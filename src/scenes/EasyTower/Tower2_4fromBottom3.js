...
export default class EasyTower2_4fromBottom3 extends Phaser.Scene
{
    // player;
    preload()
    {
    ...
    create()
    {
        ...
        .on('pointerdown', () => { 
            let paused = true
            this.scene.launch("pauseScreen",{scene:"easytower2_4frombottom3"})
            this.scene.sleep("easytower2_4frombottom3")
    ...
    update() {
        ...
        if (character.x > 785 && character.y < 250) {
            this.scene.start("easytower3from4")
        }
        if (character.y < 20) {
            this.scene.start("easytower5")
        }
        if (character.y > 480 && character.x > 400) {
            this.scene.start("easytower1from2")
        }
        if (character.y > 480 && character.x <= 400) {
            this.scene.start("easytower1from4")
        }
    }
}