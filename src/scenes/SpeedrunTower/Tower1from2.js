...
export default class SpeedrunTower1from2 extends Phaser.Scene
{
    // player;
    preload()
    {
    ...
    create()
    {
        ...
        .on('pointerdown', () => { 
            let paused = true
            this.scene.launch("pauseScreen",{scene:"speedruntower1from2"})
            this.scene.sleep("speedruntower1from2")
    ...
    update() {
        ...
        if (character.x > 785) {
            localStorage.setItem(15, this.overallDisplay);
            this.scene.start("speedrunstartingPointfrom1")
        }
        if (character.y < 5) {
            localStorage.setItem(16, this.overallDisplay);
