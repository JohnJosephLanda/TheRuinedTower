...
export default class SpeedrunTower1 extends Phaser.Scene
{
    // player;
    preload()
    {
    ...
    create()
    {
        ...
        .on('pointerdown', () => { 
            let paused = true
            this.scene.launch("pauseScreen",{scene:"speedruntower1"})
            this.scene.sleep("speedruntower1")
    ...
    update() {
        ...
        if (character.x > 785) {
            localStorage.setItem(0, this.overallDisplay);
            this.scene.start("speedrunstartingPointfrom1")
        }
        if (character.y < 5) {
            localStorage.setItem(1, this.overallDisplay);
